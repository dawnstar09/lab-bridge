...
const copy: Record<Locale, Record<string,string>> = {
  ...
  en:{intro:"Find collaborators from your proposal and researcher profile.",saved:"Project registered.",requested:"Meeting request saved.",close:"Close",funding:"Crowdfunding",fundingIntro:"Publish a plan to secure early collaborators and resources.",register:"Register project",registered:"Registered",goal:"Goal",created:"Created",empty:"No projects registered.",matching:"KAIST lab matching",matchingIntro:"Ranked by a hybrid of specialty taxonomy and cosine similarity across interests, publications, and lab descriptions.",source:"KAIST lab data",fit:"Profile fit",meeting:"Request meeting",meetingDone:"Request saved",reason:"Matching terms",low:"Add interests and publications to improve matching.",profileRequired:"Complete your research field and specialty in My page to calculate matching scores.",newProject:"Register funding project",name:"Project name",amount:"Target amount",description:"Description",submit:"Register",files:"Attachments",fileHelp:"PDF, DOCX, HWP/HWPX, XLSX, PPTX, ZIP, images · up to 20MB each",details:"View details",author:"Author",organization:"Organization",download:"Download",noFiles:"No attachments",uploading:"Registering...",projectDetail:"Project details",publicNotice:"Attachments are visible to project participants."},
...
export function NetworkClient() {
  ...
  const hasMatchingProfile = Boolean(profile?.researchField || profile?.specialty || profile?.interests || profile?.publications?.length);
  ...
  return <section className="network-page">
    ...
    <div className="network-section"><div className="card-heading"><div><span className="section-label">COSINE MATCHING</span><h2>{ui.matching}</h2><p>{ui.matchingIntro}</p></div><small className="data-source">{ui.source}</small></div>
      <p className="matching-note">{hasMatchingProfile ? ui.low : (ui.profileRequired || ui.low)}</p><div className="researcher-list">{matches.map((item,index) => { const translated = locale !== "ko"; const professor = translated ? item.professorEn : item.professor; return <article key={item.id}><div className="match-rank">{String(index+1).padStart(2,"0")}</div><div><div className="match-meta"><StatusPill tone="line">{translated ? item.departmentEn : item.department}</StatusPill><b>{ui.fit} {hasMatchingProfile ? `${item.score}%` : "—"}</b></div><h3>{translated ? item.labEn : item.lab}</h3><p><strong>{professor}</strong>{item.matchedKeywords.length && !translated ? ` · ${ui.reason}: ${item.matchedKeywords.join(", ")}` : ""}</p></div><button disabled={requestedIds.has(item.id)} onClick={() => void requestMeeting(item.id,professor)}>{requestedIds.has(item.id) ? ui.meetingDone : `${ui.meeting} →`}</button></article>; })}</div>
    </div>
