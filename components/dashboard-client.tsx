    ...
    <main className="site-page">
      ...
      <section className="mypage">
        ...
        <div className="profile-grid">
          ...
          <section className="profile-card full"><div className="card-heading"><div><h2>{t("savedPrograms")}</h2><p>{t("savedProgramsHelp")}</p></div><Link href="/rd">{t("viewAll")} →</Link></div>{savedPrograms.length ? savedPrograms.map((program) => <div className="recommendation-row" key={program.id}><div><b>{programStatus(program.status, locale)}</b><span>{program.deadline}</span></div><h3>{programText(program.title, programTranslations[program.id]?.title, locale)}</h3><Link href={`/rd/${program.id}`}>{t("check")} →</Link></div>) : <p>{t("noSavedPrograms")}</p>}</section>
          <section className="profile-card full"><div className="card-heading"><div><h2>{t("recommendedPrograms")}</h2><p>{t("recommendationHelp")}</p></div><Link href="/rd">{t("viewAll")} →</Link></div>{topRecommendation && <div className="recommendation-row"><div><b>{topRecommendation.match.score}%</b><span>{t("profileMatch")}</span></div><h3>{programText(topRecommendation.program.title, programTranslations[topRecommendation.program.id]?.title, locale)}</h3><Link href={`/rd/${topRecommendation.program.id}`}>{t("check")} →</Link></div>}</section>
        </div>
      </section>
