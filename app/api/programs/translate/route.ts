...
export async function POST(request: Request) {
  ...
  const selected = programs.filter((program) => body.ids?.includes(program.id)).slice(0, 50);
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return NextResponse.json({ error:"OPENAI_API_KEY_MISSING" }, { status:503 });
  const response = await fetch("https://api.openai.com/v1/responses", { method:"POST", headers:{ Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json" }, body:JSON.stringify({
    model:process.env.OPENAI_MODEL || "gpt-5.6-luna",
    input:`Translate these Korean R&D announcement records into ${languages[body.locale]}. Preserve acronyms, years, program identifiers and legal meaning. Use natural research-administration terminology. Return only the schema output.\n${JSON.stringify(selected.map(({id,title,ministry,agency,programName}) => ({id,title,ministry,agency,programName})))}`,
